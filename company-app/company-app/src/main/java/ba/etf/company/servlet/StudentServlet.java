...
public class StudentServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (pathInfo != null && pathInfo.matches("/\\d+")) {
        ...
        } else if (pathInfo != null && pathInfo.matches("/\\d+/progress")) {
            Long id = Long.parseLong(pathInfo.split("/")[1]);
            String response = ApiClient.get("/students/" + id, token);
            Student student = mapper.readValue(response, Student.class);
            ...
            req.setAttribute("student", student);
            req.setAttribute("applicationsJson", appsResp);
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (pathInfo != null && pathInfo.matches("/\\d+/progress/evaluate")) {
            ...
            for (int i = 0; i < types.length; i++) {
                ...
                if (!filtered.isEmpty()) {
                    ...
                    for (CvEntry entry : filtered) {
                        Paragraph entryTitle = new Paragraph(entry.getTitle(), normalFont);
